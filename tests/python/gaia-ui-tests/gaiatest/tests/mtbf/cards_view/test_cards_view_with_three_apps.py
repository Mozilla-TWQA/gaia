...
class TestCardsViewThreeApps(GaiaMtbfTestCase):
    ...
    def test_cards_view(self):
        ...
        if self.cards_view.is_app_present(self._test_apps[0]):
            self.assertFalse(self.cards_view.is_app_displayed(
            self._test_apps[0]),
            "First opened app should not be visible in cards view")

        if self.cards_view.is_app_present(self._test_apps[1]):
            self.assertTrue(self.cards_view.is_app_displayed(
            self._test_apps[1]),
            "Second app opened should be visible in cards view")

        if self.cards_view.is_app_present(self._test_apps[2]):
            self.assertTrue(self.cards_view.is_app_displayed(
            self._test_apps[2]),
